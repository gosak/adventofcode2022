...
fun main() {
    ...
    println("result ${day5_1(input)}")
}

fun day5_1(input: List<String>): String {
    ...
    val executor = Day5OrdersExecutor(storage)
    orders.forEach {
        executor(it)
    }
...
sealed class Day5Order {
    abstract fun invoke(storage: List<MutableList<Char>>)
}
...
data class Day5Move(
    ...
    override fun invoke(storage: List<MutableList<Char>>) {
        val move = storage[fromPileIndex].takeLast(cratesToMove).reversed()
